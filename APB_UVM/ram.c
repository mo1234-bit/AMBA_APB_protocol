...
static uint8_t PRDATA1 = 0;

// DPI-C function for Slave1 simulation
void slave1_sim(int PCLK, int PRESETn, int PSEL, 
    ...
    if (PCLK) {
        if (!PRESETn) {
            ...
            PREADY = 1;
            adder = PADDR & 0xFF;
            PRDATA1 = mem[adder];  // Update PRDATA only during read
        }
        else {
            PREADY =0 ;
            PRDATA1 = PRDATA1;  // Clear when idle
        }
    }
}
